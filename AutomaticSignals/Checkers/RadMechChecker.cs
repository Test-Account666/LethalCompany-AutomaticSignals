...
public static class RadMechChecker {
    ...
    public static void Initialize(ConfigFile configFile) {
        _minimumTeleportCoolDown = configFile.Bind("Old Birds", "1. Minimum teleport cooldown", 1000,
                                                   "Defines the minimum cooldown (in milliseconds) to wait before teleporting a player that is being torched");

        _maximumTeleportCoolDown = configFile.Bind("Old Birds", "2. Maximum teleport cooldown", 20000,
                                                   "Defines the maximum cooldown (in milliseconds) to wait before teleporting a player that is being torched");

        _minimumTimeWaiting = configFile.Bind("Old Birds", "3. Minimum teleport delay", 300,
                                              "Defines the minimum delay (in milliseconds) to wait before actually trying to teleport a player that is being torched");

        _maximumTimeWaiting = configFile.Bind("Old Birds", "4. Maximum teleport delay", 1200,
                                              "Defines the maximum delay (in milliseconds) to wait before actually trying to teleport a player that is being torched");

        _teleportChance = configFile.Bind("Old Birds", "5. Teleport chance", 70,
                                          new ConfigDescription("Defines the chance for teleporting a player that is being torched",
